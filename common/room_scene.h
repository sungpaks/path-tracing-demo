...
class room_checker : public material {
public:
  bool scatter(const ray& incoming, const hit_record& rec, color& attenuation,
               ray& scattered) const override {
    const int x = static_cast<int>(std::floor(rec.p.x() / 0.5));
    const int z = static_cast<int>(std::floor(rec.p.z() / 0.5));
    const color albedo = (x + z) % 2 == 0 ? color(0.72, 0.72, 0.72) : color(0.14, 0.16, 0.19);
    return lambertian(albedo).scatter(incoming, rec, attenuation, scattered);
  }
};
...
inline shared_ptr<hittable> make_room_object(room_shape shape, double x,
  ...
  case room_shape::torus:
    ...
    // Stand the torus upright so the opening is visible from the entrance.
  ...
  // Support of the transformed primitive along world Y: rest exactly on the floor.
  // dot(Y, R*S*p) = dot(S*R^-1*Y, p).
...
inline int run_room_scene(room_shape shape, int argc, char* argv[], bool transformed = false) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--offline")
      ...
      std::cerr << "Usage: " << argv[0] << " [--offline] [--preview]\n";
      return 1;
    }
  }
  ...
  // Interior: x in [-3,3], y in [0,5], z in [-3,3]. Open toward +Z.
  ...
  world.add(
      make_shared<cube>(point3(0, -0.1, 0), vec3(6.4, 0.2, 6.4), make_shared<room_checker>()));

  // Broad area emitter beneath the ceiling. No artificial sky illumination.
  world.add(make_shared<cube>(point3(0, 4.96, -0.3), vec3(2.2, 0.04, 1.8),
                              make_shared<diffuse_light>(color(8, 8, 8))));

  // Left: glass, center: diffuse, right: metal.
